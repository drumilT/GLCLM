...
def get_performance(crit, pred_logits, labels, hparams, x_len, sum_loss=True):
  # average over length
  ...
  if hparams.bt:
    pred_logits = pred_logits.view(-1, hparams.src_vocab_size)
    pred_loss = crit(pred_logits, labels)
    pred_loss = pred_loss.view(batch_size, -1).sum(-1)
    _, pred_preds = torch.max(pred_logits, dim=1)
    pred_acc = torch.eq(pred_preds, labels).int().masked_fill_(mask, 0).sum().item()
